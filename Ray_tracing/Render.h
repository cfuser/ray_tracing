...
inline Eigen::Vector3d MIS_correction(std::vector<Eigen::Vector3d> &Radiance, std::vector<double> &pdf)
{
	double sum_pdf = 0;
	for (auto single_pdf : pdf)
	{
		sum_pdf = sum_pdf + std::pow(single_pdf, MIS_correction_coefficient);
	}
	...
	{
		res = res + std::pow(pdf[i], MIS_correction_coefficient) / sum_pdf * Radiance[i];
	}
	return res;
}
...
Eigen::Vector3d shade_4(Ray r_out, int depth, BVH *bvh_root, std::vector<TriangleMesh> *LightMeshes, Scene *scene)
{
	...
	//if (false)
	if (divide_coefficient != -1)
	{
		f_r_Kd = f_r_Kd / divide_coefficient; f_r_Ks = f_r_Ks / divide_coefficient;
	}
	else
	{
		double type_num = 0;
		...
		if (f_r_Kd.sum() != 0)
			type_num += 1;
		f_r_Kd = f_r_Kd / type_num;
		f_r_Ks = f_r_Ks / type_num;
	}
	// !!!important choice
	...
	std::vector<Eigen::Vector3d> Dir_Light_Radiance_List;
	std::vector<double> Dir_Light_pdf_List;
	...
	{
		Eigen::Vector3d single_res = Eigen::Vector3d::Zero();
		...
		cos_theta_p = std::max(cos_theta_p, 0.0);
		double cos_theta_x = x_vn.dot(-dir);
		cos_theta_x = std::max(cos_theta_x, 0.0);
		if (sample_all_light == 1)
			light_pdf = 1.0 / get_area(light_mesh.v);
		else if (sample_by_group == 1)
		{
			...
			if (false)
				if (MIS_Method == 1)
					light_pdf = light_pdf * (double)(scene->LightIndex[i].second - scene->LightIndex[i].first) / (double)scene->lightmesh_area.size();
		}
		else
		{
		...
		if (abs(block.distance - distance) < EPSILON)
		{
			...
			Eigen::Vector3d Kd_Radiance = light_mesh.material->light_attr->Radiance.cwiseProduct(f_r_Kd) * cos_theta_p * cos_theta_x / std::pow(distance, distance_coefficient) / light_pdf / M_PI;
			//std::cout << i << std::endl << temp_ << std::endl;
			Sample_Light += Kd_Radiance;
			single_res += Kd_Radiance;
			...
			if (intersection.mesh->material->Ns != 1)
			{
				...
				Eigen::Vector3d Ks_Radiance;// = Eigen::Vector3d::Ones();
				// !!! important Ks_Radiance = f_r_Ks * (intersection.mesh->material->Ns + 2) / (intersection.mesh->material->Ns + 1) * cos_theta_p;
				...
				// !!! important if (specular_large_coefficient == 1)
				// !!! important 	Ks_Radiance = Ks_Radiance * (intersection.mesh->material->Ns + 1) / 2.0 / M_PI * fast_power(cos_alpha, intersection.mesh->material->Ns);
				// !!! important Ks_Radiance = Ks_Radiance.cwiseProduct(light_mesh.material->light_attr->Radiance) * cos_theta_x / std::pow(distance, distance_coefficient) / light_pdf;
				...
				Ks_Radiance = f_r_Ks.cwiseProduct(light_mesh.material->light_attr->Radiance) * fast_power(std::max(0.0, cos_alpha), intersection.mesh->material->Ns) / light_pdf * (intersection.mesh->material->Ns + 2) / 2.0 / M_PI * cos_theta_p * cos_theta_x / std::pow(distance, distance_coefficient);
				if (control_rho_s)
					Ks_Radiance = Ks_Radiance.cwiseProduct(rho_s_vec_inv).cwiseProduct(rho_s_vec_contribution);
				single_res += Ks_Radiance;
				Sample_Light += Ks_Radiance;
			}
			//if (MIS_Method == 1 && sample_by_group == 1)
			//	single_res = single_res / (double)(scene->LightIndex[i].second - scene->LightIndex[i].first) * (double)scene->lightmesh_area.size();
			Dir_Light_Radiance_List.push_back(single_res);
			if (I_think_correct)
				light_pdf = light_pdf * std::pow(distance, distance_coefficient) / cos_theta_x * 2 * M_PI;
			if (I_think_correct_further)
				light_pdf = light_pdf * std::pow(distance, distance_coefficient) / cos_theta_x;
			if (single_res[0] < 0 || single_res[1] < 0 || single_res[2] < 0)
			{
				std::cout << "exists single_res[i] < 0" << std::endl;
				std::cout << "MIS_triangle_mesh_unifrom_hit_sample" << std::endl;
				system("pause");
			}
			if (light_pdf == 0)
			{
				std::cout << "light_pdf == 0" << std::endl;
				std::cout << "MIS_triangle_mesh_uniform_hit_sample" << std::endl;
				system("pause");
			}
			Dir_Light_pdf_List.push_back(light_pdf);
		}
		else if (MIS_triangle_unhit_sample)
		{
			Dir_Light_Radiance_List.push_back(Eigen::Vector3d::Zero());
			if (_paper)
				light_pdf = light_pdf * std::pow(distance, distance_coefficient) / cos_theta_x / cos_theta_p;
			else
			{
				if (I_think_correct)
					light_pdf = light_pdf * std::pow(distance, distance_coefficient) / cos_theta_x * 2 * M_PI;
			...
			{
				std::cout << "exists single_res[i] < 0" << std::endl;
				std::cout << "MIS_triangle_mesh_unifrom_unhit_sample" << std::endl;
				system("pause");
			}
			if (light_pdf == 0)
			{
				std::cout << "light_pdf == 0" << std::endl;
				std::cout << "MIS_triangle_mesh_uniform_unhit_sample" << std::endl;
				system("pause");
			}
			Dir_Light_pdf_List.push_back(light_pdf);
		}
	...
	if (MIS_BRDF == 1 && intersection.mesh->material->Ns != 1)
	//if (sample_dir_light == 1 && sample_all_light == 0 && intersection.mesh->material->Ns != 1)
	{
		Eigen::Vector3d n = intersection.normal;
		...
		if (wi.dot(intersection.normal) > 0)
		{
			...
			cos_theta_i = std::max(cos_theta_i, 0.0);
			double cos_theta_o = intersection.normal.dot(-r_out.direction);
			cos_theta_o = std::max(cos_theta_o, 0.0);

			Intersection q = Intersect(r_in, bvh_root);
			Eigen::Vector3d Kd_Radiance = Eigen::Vector3d::Zero();
			Eigen::Vector3d Ks_Radiance = Eigen::Vector3d::Zero();

			if (q.happened == true && q.mesh->material->light == true)
			{
				Kd_Radiance = q.mesh->material->light_attr->Radiance;
				Ks_Radiance = q.mesh->material->light_attr->Radiance;
			}
			else if (q.happened == true && q.mesh->material->light == false && specular_unhit_continue == 1)
			{
				double temp_psi = get_random_double();
				if (temp_psi < P_RR)
				{
					Kd_Radiance = shade_4(r_in, depth + 1, bvh_root, LightMeshes, scene) / P_RR;
					Ks_Radiance = Kd_Radiance;
				}
			}
			else
			{
				Kd_Radiance = Eigen::Vector3d::Zero();
				Ks_Radiance = Eigen::Vector3d::Zero();
			}

			Kd_Radiance = Kd_Radiance.cwiseProduct(f_r_Kd) * cos_theta_i / M_PI / pdf;
			BRDF += Kd_Radiance;

			if (specular_large_coefficient == 1)
				Ks_Radiance = Ks_Radiance * (intersection.mesh->material->Ns + 1) / 2.0 / M_PI * fast_power(cos_alpha, intersection.mesh->material->Ns);

			Ks_Radiance = Ks_Radiance * (intersection.mesh->material->Ns + 2) / (intersection.mesh->material->Ns + 1) * cos_theta_i;
			Ks_Radiance = Ks_Radiance.cwiseProduct(f_r_Ks) / pdf / P_RR;
			Eigen::Vector3d rho_s_vec_inv = rho_s_vec.cwiseInverse();
			//shading_res = shading_res / rho_s_vec[color_type];

			// to calculate '/ rho_s'
			if (control_rho_s)
			Ks_Radiance = Ks_Radiance.cwiseProduct(rho_s_vec_inv);

			//shading_res = shading_res.cwiseProduct(rho_s_vec);
			if (control_rho_s)
			Ks_Radiance = Ks_Radiance.cwiseProduct(rho_s_vec_contribution);
			...
			if (q.happened == true && q.mesh->material->light == true)
			{
				Dir_Light_pdf_List.push_back(pdf);
				Dir_Light_Radiance_List.push_back(BRDF);
			}
			else if (q.happened == true && q.mesh->material->light == false && specular_unhit_continue == 1)
			{
				Dir_Light_pdf_List.push_back(pdf);
		...
		else if (MIS_specular_unhit_sample)
		{
			Dir_Light_pdf_List.push_back(pdf);
			Dir_Light_Radiance_List.push_back(BRDF);
		}
	...
	Eigen::Vector3d Diffuse_Radiance;

	if (MIS_Diffuse == 1 && intersection.mesh->material->Ns == 1)
		//if (sample_dir_light == 1 && sample_all_light == 0 && intersection.mesh->material->Ns != 1)
	{
		Eigen::Vector3d wi;
		double pdf;
		double cos_theta_i;
		Eigen::Vector3d single_res;
		...
		Eigen::Vector3d r_in_dir = 2 * (-r_out.direction.dot(n)) * n - -r_out.direction;
		r_in_dir = r_in_dir.normalized();
		double cos_alpha = wi.dot(r_in_dir);
		cos_alpha = std::max(cos_alpha, 0.0);

		Ray r_in(intersection.coords, wi); r_in.mesh = intersection.mesh; r_in.Ni = intersection.mesh->material->Ni; r_in.in_material = r_out.in_material;
		Intersection q = Intersect(r_in, bvh_root);
		Eigen::Vector3d shading_res = Eigen::Vector3d::Zero();
		Eigen::Vector3d Kd_Radiance, Ks_Radiance;
		if (q.happened == true && q.mesh->material->light == true)
		{
			Kd_Radiance = q.mesh->material->light_attr->Radiance;
			Ks_Radiance = q.mesh->material->light_attr->Radiance;
		}
		else if (q.happened == true && q.mesh->material->light == false && diffuse_unhit_continue == 1)
		{
			double temp_psi = get_random_double();
			if (temp_psi < P_RR)
			{
		...
		if (pdf == 0)
		{
			...
			std::cout << "MIS_Diffuse_sample" << std::endl;
			system("pause");
		}
		if (single_res[0] < 0 || single_res[1] < 0 || single_res[2] < 0)
		{
		...
		if (q.happened == true && q.mesh->material->light == false && diffuse_unhit_continue == 1)
		{
			Dir_Light_Radiance_List.push_back(single_res);
			Dir_Light_pdf_List.push_back(pdf);
		}
		else if (MIS_diffuse_unhit_sample)
		{
			Dir_Light_Radiance_List.push_back(single_res);
			Dir_Light_pdf_List.push_back(pdf);
		}
	...
	if (MIS_sample_area)
	{
		...
		//int light_mesh_index = Sample_Light(LightMeshes);
		int light_mesh_index;
		light_mesh_index = Sample_Light_Area(&scene->lightmesh_area);
		//std::cout << light_mesh_index << std::endl;
		TriangleMesh light_mesh = (*LightMeshes)[light_mesh_index];
		// Uniformly sample the light at x' (pdf_light = 1 / A);
		// Shoot a ray from p to x';
		...
		cos_theta_x = std::max(cos_theta_x, 0.0);
		light_pdf = 1.0 / scene->lightmesh_area.back();
		light_pdf = 1.0 / get_area(light_mesh.v) / scene->lightmesh_area.size();
		if (abs(block.distance - distance) < EPSILON)
		{
			...
			if (single_res[0] < 0 || single_res[1] < 0 || single_res[2] < 0)
			{
				std::cout << "exists light_pdf[i] < 0" << std::endl;
				std::cout << "MIS_triangle_area_unifrom_hit_sample" << std::endl;
				system("pause");
			}
			if (light_pdf == 0)
			{
		...
		else if (MIS_triangle_unhit_sample)
		{
			...
			{
				std::cout << "exists single_res[i] < 0" << std::endl;
				std::cout << "MIS_triangle_area_unifrom_unhit_sample" << std::endl;
				system("pause");
			}
			if (light_pdf == 0)
			{
		...
		if (intersection.mesh->material->Ni == r_out.Ni && r_out.in_material == false)
		{
			if (psi <= P_RR)
			{
				//if (random_xi < rho_d)
				{
					...
					//if (false)
					if (q.happened == true && q.mesh->material->light == true && (sample_dir_light == 1 || sample_by_group == 1 || sample_all_light == 1 || MIS_BRDF == 1 || MIS_sample_area == 1 || MIS_Diffuse == 1))
						shading_res = Eigen::Vector3d::Zero();
					else
						shading_res = shade_4(r_in, depth + 1, bvh_root, LightMeshes, scene);

					if (false)
					if ((sample_dir_light == 0 || (q.happened == true && q.mesh->material->light != true)) && MIS_sample_area == 0)
						shading_res = shade_4(r_in, depth + 1, bvh_root, LightMeshes, scene);
					else
						shading_res = Eigen::Vector3d::Zero();
				...
				if (specular_exists == 1)
					if (f_r_Ks[0] != 0 || f_r_Ks[1] != 0 || f_r_Ks[2] != 0)
					{
						...
						if (wi.dot(intersection.normal) > 0)
						{
							...
							//if (false)
							if (q.happened == true && q.mesh->material->light == true && (sample_dir_light == 1 || sample_by_group == 1 || sample_all_light == 1 || MIS_BRDF == 1 || MIS_sample_area == 1))
								shading_res = Eigen::Vector3d::Zero();
							else
								shading_res = shade_4(r_in, depth + 1, bvh_root, LightMeshes, scene);
	...
	if (MIS_Method == 1)
	{
		L_dir = MIS_correction(Dir_Light_Radiance_List, Dir_Light_pdf_List);
	}
	else
	{
		L_dir = Sample_Light;
	...
	if (L_dir[0] < 0 || L_dir[1] < 0 || L_dir[2] < 0)
	{
		std::cout << "exists L_dir[i] < 0" << std::endl;
		system("pause");
	}
	if (L_indir[0] < 0 || L_indir[1] < 0 || L_indir[2] < 0)
	{
		std::cout << "exists L_indir[i] < 0" << std::endl;
		system("pause");
	}
	return L_dir + L_indir;
}
