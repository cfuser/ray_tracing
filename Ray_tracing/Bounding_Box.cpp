...
Bounding_Box Bounding_Box::Union(Bounding_Box add)
{
	Bounding_Box res;
...
double Bounding_Box::get_surface_area()
{
	this->surface_area = (x_max - x_min) * (y_max - y_min) + (y_max - y_min) * (z_max - z_min) + (z_max - z_min) * (x_max - x_min);
	return surface_area;
}

bool Bounding_Box::IntersectP(Ray &ray, Eigen::Vector3d invDir, std::array<int, 3>& dirIsNeg)
{
