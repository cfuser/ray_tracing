...
void BVH::Triangle_Barycentric_Render(cv::Mat *res)
{
...
void BVH::Print(int &total_mesh, int *vis)
{
...
int partition(double *dim_p, int left, int right)
{
	...
	return r;
	*/
	int i = get_random_double() * (right - left + 1) + left;
	std::swap(dim_p[i], dim_p[right]);

	double x = dim_p[right];
	i = left - 1;
	for (int j = left; j < right; ++j) {
		if (dim_p[j] <= x) {
			std::swap(dim_p[++i], dim_p[j]);
		}
	}
	std::swap(dim_p[i + 1], dim_p[right]);
	return i + 1;
}
...
void divide_index(std::vector<TriangleMesh> *meshes, int index, int p, std::vector<TriangleMesh> &child_L, std::vector<TriangleMesh> &child_R)
{
...
void divide_mid(std::vector<TriangleMesh> *meshes, double mid, int p, std::vector<TriangleMesh> &child_L, std::vector<TriangleMesh> &child_R)
{
...
void SAH(std::vector<TriangleMesh> *meshes, double &mid, int &p, int bucket_number)
{
	mid = -DBL_MAX;
	p = -1;
	double loss = DBL_MAX;
...
BVH* building(BVH* bvh, std::vector<TriangleMesh> *meshes)
{
	...
	if (_SAH)
	{
		double mid = 0;
		SAH(meshes, mid, p);
		divide_mid(meshes, mid, p, child[0], child[1]);
	}
	else
	{
		int index = get_median_p(meshes, p);
		divide_index(meshes, index, p, child[0], child[1]);
	}
...
Intersection getIntersection(Ray &ray, TriangleMesh *mesh)
{
	Intersection res;
...
{
	//std::cout << "Intersect: ray direction : " << std::endl << ray.direction << std::endl;
